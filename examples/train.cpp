...
void save_model(const std::string& filename, const std::vector<std::pair<std::string, std::shared_ptr<Tensor>>>& params) {
    ...
    for (const auto& pair : params) {
        ...
        file.write((char*)data, data_size * sizeof(float));
    }
...
int main() {
    std::cout << "Initializing Arena Allocator..." << std::endl;

    size_t arena_size = 1024 * 1024 * 512;  // 512MB
    void* buffer = malloc(arena_size);
    if (!buffer) {
        std::cerr << "Failed to allocate arena buffer" << std::endl;
        return 1;
    }
    Arena transient_arena = arena_init(buffer, arena_size);
    std::cout << "   Transient Arena initialized with " << arena_size / (1024 * 1024) << " MB" << std::endl;
    ...
    size_t split_idx = static_cast<size_t>(total_load * 0.8);
    ...
    Conv2D conv1(1, 12, 3, 1, 1, nullptr);
    Relu relu1;
    Pooling pool1(2, 2);
    Dropout drop1(0.25f);

    Conv2D conv2(12, 24, 3, 1, 1, nullptr);
    Relu relu2;
    Pooling pool2(2, 2);
    Dropout drop2(0.25f); 
    ...
    // 0.0002f is ideal since gradients accumulate across 32 steps before matching parameter updates
    SGD optimizer(p1, 0.0002f); 

    CrossEntropyLoss criterion;

    std::vector<size_t> indices(train_x.size());
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (size_t step = 0; step < train_x.size(); step++) {
            size_t i = indices[step]; // Extract randomized item sequence reference
            ...
            // Backward Pass (Safely stacks modifications inside target weights)
            loss->backward(); 

            // Step optimizer and clear temporary activation blocks ONLY at batch completeness boundaries
            if ((step + 1) % BATCH_SIZE == 0 || step == train_x.size() - 1) {
                optimizer.step();
                arena_free_all(&transient_arena); 
            }

            // Accuracy tracking metrics extraction
            const auto* logits = out->data();
            int pred = static_cast<int>(std::distance(logits, std::max_element(logits, logits + 7)));
            if (pred == train_y[i]) train_correct++;
        }

        // --- Validation Execution (Turn off Dropout tracks) ---
        drop1.eval(); 
        drop2.eval();
        ...
        float train_acc = static_cast<float>(train_correct) / train_x.size() * 100.0f;
        float val_acc = static_cast<float>(val_correct) / val_x.size() * 100.0f;
    ...
    save_model("models/fer_model.bin", p1);
